...
```
```java
implementation 'com.github.Rongzhi-maker:Coroutine:1.2.9.07'
...
![时序图](https://github.com/Rongzhi-maker/Coroutine/blob/master/liuchengtu.jpg)
...
> 行为说明（1.2.9.04+）  
> 1. `cancel()` 是尽力取消：会优先标记流为取消，并停止后续分发；但已经开始执行的任务不保证被强制中断。  
> 2. `Observable` 为晚订阅补发保留了固定长度历史缓存，当前最多保留最近 5 条结果（超出会淘汰最旧数据）。  
> 3. 若未设置 `error(...)`，事件流内部异常会根据全局配置 `SetEscapeHandler` / `SetIgnoreCrash` 决定是否继续抛出。  
...
> 说明：`SetCodeStr/SetMsgStr` 开启预解析后，默认约定 `code == 0` 为成功，`code != 0` 会进入错误分支。  
